...
* [`src/message_log_3/`](src/message_log_3/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/message_log_3/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `message-log-3` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/message_log_3/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns message-log-3.example
  (:require [message-log-3.config :as config])
